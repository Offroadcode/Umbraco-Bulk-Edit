angular.module("umbraco.resources").factory("bulkEditApi", function($http) {
    return {
        ...
        getAllContentTypes: function() {
           return $http.get("/Umbraco/backoffice/ORCCsv/ContentType/getAll"); 
        },
...
angular
    .module("umbraco")
    .controller(
        "bulkEdit.dashboard.controller",
        function($scope, appState, bulkEditApi, contentResource, dataTypeResource, dialogService, navigationService, notificationsService) {
        ...
        */
         ...
         */
            ...
            $scope.onDeleteSavedSearchDialogConfirmation = function(data) {
                bulkEditApi
                    .deleteSavedSearchByGuid(data.guid)
        ...
        */
         ...
         */
            $scope.saveAll = function() {
                ...
                for (
                    var i = $scope.currentPage * perPage;
                    i < ($scope.currentPage + 1) * perPage;
                    i++
                ) {
                    if ($scope.results[i]) {
                        ...
                        for (
                            var j = 0;
                            j < $scope.propertiesToEdit.length;
                            j++
                        ) {
         ...
         */
            $scope.search = function() {
                $scope
                    .getContent($scope.startNode, $scope.doctype.Alias)
         ...
         * the result.
         */
            $scope.addEditorForPropertyToResultItem = function(
                propertyAlias,
                editor,
                index
            ) {
         ...
         */
            $scope.getContent = function(node, doctypeAlias) {
                return bulkEditApi
                    .getMatchingContent(node.id, doctypeAlias)
                    .then(
                        function(response) {
                        ...
                        function(error) {
                            console.error(
                                "Error with getContent() in bulkEdit.dashboard.controller.js: ",
                                error
                            );
                        }
                    );
         ...
         */
            $scope.getCurrentSavedSearchPage = function() {
                ...
                var perPage = $scope.config.itemsPerPage;
                if (
                    $scope.savedSearches.length >
                    $scope.currentSavedSearchPage * perPage
                ) {
                    for (
                        var i = $scope.currentSavedSearchPage * perPage;
                        i < ($scope.currentSavedSearchPage + 1) * perPage;
                        i++
                    ) {
         ...
         */
            $scope.saveSearchIfUnique = function(startNode, docType) {
                ...
                for (var i = 0; i < $scope.savedSearches.length; i++) {
                    ...
                    var options = JSON.parse(search.Options);
                    if (
                        options.rootId == startNode.id &&
                        options.alias == docType.Alias
                    ) {
                        isUnique = false;
                    }
                }
